...
/**
 * Obtém a connection string do tenant via Admin API (com cache).
 */
export async function getTenantConnectionString(tenantId: string): Promise<string> {
  ...
  const url = `${adminApiConfig.url.replace(/\/$/, '')}/provisioning/tenant/${tenantId}/connection?module=internal_chat`;
...
/**
 * Executa uma função com um client pg do tenant. O client é fechado ao final.
 */
